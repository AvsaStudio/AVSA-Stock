...
const { generateAllPrices, generateNews, generateMarketSummary } = require('./dataGenerator');
...
app.get('/health', (_req, res) => {
  res.json({
    ...
    services: {
      ...
      cassandra: process.env.CASSANDRA_CONTACT_POINTS ? 'configured' : 'not configured',
    },
  });
});
...
setInterval(async () => {
  ...
  const message = JSON.stringify({ type: 'price_update', data: prices, timestamp: new Date().toISOString() });
...
setInterval(async () => {
  ...
  const message = JSON.stringify({ type: 'news_update', data: news, timestamp: new Date().toISOString() });
  for (const client of clients) {
    if (client.readyState === WebSocket.OPEN) {
      client.send(message);
    }
